...
def cal_95CI(year, columns, col_names, colors, ax,labels,shift):

    for idx, (ts, clr, name,mylabel) in enumerate(zip(columns, colors, col_names,labels)):
    ...
    # -----------------------------------------------------------------------------
        ...
        ax.plot(year, ts,marker='.',  linestyle='', color=clr,label=mylabel)
        ...
        ax.text(0.2+shift,0.2, mylabel+ str_bL + '$\pm$' + str_b95CI+' mm/year', ha='center', va='center', transform=ax.transAxes,alpha=1,fontsize=12,backgroundcolor='1')


        # ax.text(0.1, 0.07, '$SE_b$= '+ str_SEb + ' mm/year', ha='center', va='center', transform=ax.transAxes)
        # ax.text(0.3, 0.07, '$SE_{bc}$= '+ str_SEbc + ' mm/year', ha='center', va='center', transform=ax.transAxes)
        # ax.text(0.7, 0.07, 'Calculated vs. Projected 95%CI: '+ str_b95CI + ' vs. '+ str_b95CI_mod + ' mm/year', ha='center', va='center', transform=ax.transAxes)
...
def plot_combined_dataframes(dfs, colors,name_df):
    fig, axes = plt.subplots(3, 1, figsize=(10, 6))
    combined_min = float('inf')
    combined_max = float('-inf')

    
    for ax, col_name in zip(axes, dfs[0].columns[1:4]):
        shift=0
        combined_values = []
        for idx, df in enumerate(dfs):
            ...
            if name_df[idx].startswith('G'):
                labels = ["GOM20"]
                color = 'black'
            elif name_df[idx].startswith('H'):
                labels = [" HRF20"]
                color = 'blue'
            elif name_df[idx].startswith('I'):
                labels = [" IGS14"]
                color = 'purple'
            else:
                labels = [name_df[idx]]
            ...
            cal_95CI(year, [columns], [col_name], [color], ax, labels,shift)
            shift += 0.2

            combined_values.extend(columns)
            combined_min = min(combined_min, min(columns))
            combined_max = max(combined_max, max(columns))


        ax.set_ylim(bottom=combined_min*1.2, top=combined_max*1.2)
        if ax == axes[-1]:  # Check if not the last subplot
         ax.legend(loc='upper center', bbox_to_anchor=(0.5, -0.2), ncol=3, fontsize='small')
         ax.spines['bottom'].set_visible(True) 
         ax.tick_params(axis='x', which='both', bottom=True, top=False, labelbottom=True)  # Hide x-axis ticks
        else:
    ...
    plt.suptitle("Displacement Rate using GPS") 
    plt.show()


 
plot_combined_dataframes(df, ['k', 'blue'],df_name)
