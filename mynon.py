...
def cal_95CI(year,columns,col_name):
   fig, axes = plt.subplots(3, 1, figsize=(10, 6))

   for mynum, ts in enumerate(columns):
    DIR = col_name[mynum]
...
# -----------------------------------------------------------------------------
    ...
    print('mydate')
    print(year)
    axes[mynum].plot(year, Ri,'k.')
    axes[mynum].plot(year,NLi, 'r.')
    axes[mynum].set_ylabel(DIR)
    axes[mynum].spines['top'].set_visible(False)    # Remove top border
    axes[mynum].spines['right'].set_visible(False)  # Remove right border
    axes[mynum].spines['bottom'].set_visible(False)

    if mynum != len(columns) - 1:  # For all subplots except the last one
     axes[mynum].tick_params(axis='x', which='both', bottom=False, labelbottom=False)  # Remove x-axis and labels

    axes[mynum].set_ylim(bottom=min(ts)*1.2, top=max(ts)*1.2)
    ...
    axes[mynum].text(0.2, 0.2, 'Site velocity: '+ str_bL + '$\pm$' + str_b95CI+' mm/year', ha='center', va='center',color='black',transform=axes[mynum].transAxes,backgroundcolor='1',alpha=1)
    # axes[mynum].text(0.1, 0.07, '$SE_b$= '+ str_SEb + ' mm/year', ha='center', va='center', transform=axes[mynum].transAxes)
    # axes[mynum].text(0.3, 0.07, '$SE_{bc}$= '+ str_SEbc + ' mm/year', ha='center', va='center', transform=axes[mynum].transAxes)
    # axes[mynum].text(0.7, 0.07, 'Calculated vs. Projected 95%CI: '+ str_b95CI + ' vs. '+ str_b95CI_mod + ' mm/year', ha='center', va='center', transform=axes[mynum].transAxes)
...
for fin in os.listdir(directory):
    if fin.endswith(".col"):
       ...
       col_list = ts_enu.columns[1:].tolist()

       year = ts_enu.iloc[:,0]    # decimal year
    
       columns = [ts_enu.iloc[:,1], ts_enu.iloc[:,2], ts_enu.iloc[:,3]]
       #result_NS=cal_95CI(year,dis,GNSS,DIR='NS',output='on', pltshow='on')
       cal_95CI(year,columns,col_list)
