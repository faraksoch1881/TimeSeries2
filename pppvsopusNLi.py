...
def cal_95CI(year, columns, col_names, colors, ax,labels,shift,myk,mysize,nal):
    ...
    # -----------------------------------------------------------------------------
        ...
        print(mylabel)
        print(nal)
        ax.plot(year, Ri-nal,marker='.',  linestyle='', color=clr,label=mylabel,alpha=myk,markersize=10)
        ax.plot(year, NLi-nal, 'r')
        ...
        ax.text(0.2+shift,0.2, mylabel+ str_bL + '$\pm$' + str_b95CI+' mm/year', ha='center', va='center', transform=ax.transAxes,alpha=1,fontsize=4,backgroundcolor='1')
...
def plot_combined_dataframes(dfs, colors,name_df):
    ...
    for ax, col_name in zip(axes, dfs[0].columns[1:4]):
        ...
        for idx, df in enumerate(dfs):
            ...
            if name_df[idx].startswith('O'):
                labels = ["OPUS"]
                color = 'black'
            elif name_df[idx].startswith('P'):
                labels = [" PPP"]
                color = 'blue'
            else:
                labels = [name_df[idx]]
            ...
            cal_95CI(year, [columns], [col_name], [color], ax, labels,shift,myk,mysize,nal)
            shift += 0.2
            myk +=0.5
            mysize -=4
            nal+=1
        ...
        if ax == axes[-1]:  # Check if not the last subplot
         ax.legend(loc='upper center', bbox_to_anchor=(0.5, -0.2), ncol=2, fontsize='small')
